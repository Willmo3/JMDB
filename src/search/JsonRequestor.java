...
public final class JsonRequestor {
    ...
    /**
     * Queries the IMDb API for a JSON text containing a list of films that
     * match the search criteria. Searches are based on title.
     * 
     * @param queryText the text to be searched for using the IMDb API
    ...
    /**
     * Queries the IMDb API for a JSON text containing a list of films that
     * match the search criteria. Searches are based on title the passed
     * queryType. Acceptable queryTypes are defined in the QueryTypes enumerated
     * class.
     * 
     * @param queryType the type of query to base the search on
     * @param queryText the text to be searched for using the IMDb API
     * @return a MovieList containing the movies defined by the received JSON
     */
    ...
    public static double queryRating(String id) {
        String url = String.format(
                "https://imdb-api.com/en/API/Ratings/k_mcx0w8kk/%s", id);
        try {
    ...
    public static String queryTrailer(String id) {
        String url = String.format(
                "https://imdb-api.com/en/API/YouTubeTrailer/k_mcx0w8kk/%s", id);
        try {
            ObjectMapper mapper = new ObjectMapper();
            InputStream stream = fetch(url);
            JsonNode ratings = mapper.readTree(stream);
     ...
     *
     * @param id the movie ID to get the Awards of
     * @return the IMDb rating of the passed movie
     */
    public static String queryAwards(String id) {
        String url = String.format(
                "https://imdb-api.com/en/API/Awards/k_mcx0w8kk/%s", id);
        try {
            ObjectMapper mapper = new ObjectMapper();
            InputStream stream = fetch(url);
            ...
            String outcome = "";
            String category = "";
            JsonNode list = awards.get("items");
            String result = "";
            
                event = list.get(0).get("eventTitle").asText();
                year = list.get(0).get("eventYear").asText();
                outcome = list.get(0).get("outcomeItems").get(0).get(
                        "outcomeTitle").asText();
                category = list.get(0).get("outcomeItems").get(0).get(
                        "outcomeCategory").asText();
     ...
     * raw-data JSON file to be present at the location in order to duplicate
     * its InputStream to be parsed for information.
     * 
     * @param url a String containing the URL web address that itself contains a
     ...
     * its InputStream to be parsed for information.
     * 
     * @param url the url containing a raw-data JSON file to connect to
     * @return an InputStream containing the pulled JSON information
     */
